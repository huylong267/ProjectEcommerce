...
public interface iProductDetailRepository extends CrudRepository<ProductDetail,Integer> {
    ...
    @Query(value = "select p.quantity from  tbl_productdetail p where p.product_id=:productId " +
            "and p.size_id = sizeId and p.color_id = : colorId ",nativeQuery = true)
    int getQuantity (@Param("productId") int productId , @Param("sizeId") int sizeId, @Param("colorId") int colorId);
}
